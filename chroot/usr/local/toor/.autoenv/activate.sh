...
autoenv_init() {
	...
	_mountpoint="${PWD}" # mcchae: for docker endless loop error!
...
#		\cat -e "${_envfile}" | \sed 's/.*/autoenv:     &/' # busybox cat does not support -e option # mcchae
		\cat "${_envfile}" | \sed 's/.*/autoenv:     &/'
		\echo "autoenv:"
